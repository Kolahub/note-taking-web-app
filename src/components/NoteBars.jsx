...
function NoteBars() {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      ...
      if (data) {
        dispatch(noteAction.allNotes(data));
...
<div className="px-4 py-5 border-r-2 h-full flex flex-col">
  <button
    className="flex justify-center w-full py-3 rounded-lg bg-blue-500 active:scale-95 text-white"
  ...
  >
    ...
    {
      newNoteI && <div className="bg-gray-100 text-lg font-semibold rounded-lg p-2 mt-4">
        Untitled Note
      </div>
    }
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-1">
              {note.tags.split(",").map((tag, i) => (
                <div
                  key={i}
                  className="bg-gray-200 brightness-90 rounded-md px-[6px] py-[2px] text-sm"
                >
            ...
            <div>
              <p className="text-sm">{format(note.created_at, "dd MMM yyyy")}</p>
            </div>
          </div>
        </button>
      ))
